 ...
 * @constructor
 *
 * @param {string[]} funcNames Array of function names available to migrations
 */
function Validator(funcNames) {
...
Validator.prototype.validateEntry = function(entry, fn) {
  ...
  async.eachSeries(['func', 'src', 'dst'], function(key, next) {
    if (!entry[key]) {
...
Validator.prototype._getEntryFormatError = function(entry) {
  ...
  if (this.funcNames.indexOf(entry.func) === -1) {
    return new Error(util.format('Invalid func', entry));
  }
